...
class BrowserFragment : IWebViewLifecycleFragment() {
    ...
    private var mLayoutwelcomeLayer0: RelativeLayout? = null
    private var mLoadingProgressTip: TextView? = null

    public fun hideWelcomeLayer()
    {
        mLayoutwelcomeLayer0?.visibility=View.GONE
    }
    public fun setLoadingProgressTip(value: Int)
    {
        mLoadingProgressTip?.setText("loading... %"+value)
    }
    ...
    private val onNavigationEvent = { event: NavigationEvent, value: String?,
            _: InlineAutocompleteEditText.AutocompleteResult? ->
        when (event) {
            ...
            NavigationEvent.LOAD_URL -> {
                //(activity as startActivity).onTextInputUrlEntered(value!!, autocompleteResult!!, UrlTextInputLocation.MENU)

                setOverlayVisibleByUser(false)
            }
            NavigationEvent.LOAD_TILE -> {
                (activity as FireTVActivity).onNonTextInputUrlEntered(value!!)
                setOverlayVisibleByUser(false)
            }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        ///////add by kuan////
        mLayoutwelcomeLayer0 =layout.layer0_welcome
        mLoadingProgressTip=layout.textView_loadingprogress
